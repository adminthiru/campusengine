...
// Modal
// dismissable=false forces an explicit action: backdrop click is ignored and
// the close (X) button is hidden.
export function Modal({ open, onClose, title, children, size = '', footer, dismissable = true }) {
  if (!open) return null;
  return (
    <div className="modal-overlay" onClick={e => { if (dismissable && e.target === e.currentTarget) onClose(); }}>
      <div className={`modal ${size ? `modal-${size}` : ''}`}>
        <div className="modal-header">
          <h3 className="text-18-bold">{title}</h3>
          {dismissable && <button className="btn btn-secondary btn-icon btn-sm" onClick={onClose}><X size={16} /></button>}
        </div>
...
// Loading spinner (kept for buttons / inline use)
