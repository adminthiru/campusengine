...
const createEmployee = async (req, res) => {
  try {
    ...
    const autoId = `EMP${school.code}${String(count + 1).padStart(4, '0')}`;
...
// Preview the next auto employee ID for the form.
const suggestEmployeeId = async (req, res) => {
  try {
    const school = await School.findById(req.user.school).select('code');
    const count = await Employee.countDocuments({ school: req.user.school });
