...
const createStudent = async (req, res) => {
  try {
    ...
    // Format: ADM<year><4-digit per-year sequence>, e.g. ADM20260001.
    // Use max existing sequence (not count) so deletions never cause collisions.
    const year = new Date().getFullYear();
    const prefix = `ADM${year}`;
    const existing = await Student.find({ school: schoolId, admissionNumber: new RegExp(`^${prefix}`) })
      .select('admissionNumber').lean();
    const maxSeq = existing.reduce((m, s) => {
      const seq = parseInt(s.admissionNumber.slice(prefix.length)) || 0;
      return Math.max(m, seq);
    }, 0);
    const autoAdmission = `${prefix}${String(maxSeq + 1).padStart(4, '0')}`;
...
const getStudents = async (req, res) => {
  try {
    ...
    else query.status = { $nin: ['transferred', 'inactive'] };
    if (search) {
...
const suggestCodes = async (req, res) => {
  try {
    ...
    const admissionNumber = `${prefix}${String(maxSeq + 1).padStart(4, '0')}`;
    let rollNumber = '';
    if (classId) {
      const cls = await Class.findOne({ _id: classId, school: schoolId }).select('name section').lean();
      if (cls) {
